...
export const detectParser = async (file: File): Promise<ParserDetectionResponse> => {
  const formData = new FormData()
  formData.append('file', file)
...
export const previewCSVImport = async (
  ...
  const response = await api.post('/api/v1/transactions/import-csv/preview', formData, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  })
...
export const confirmCSVImport = async (
  ...
  const response = await api.post('/api/v1/transactions/import-csv/confirm', request)
  return response.data
}
...
export const getSankeyData = async (
  ...
  const response = await api.get(`/api/v1/analytics/sankey?${params.toString()}`)
  return response.data
}
...
export const getImportSessionTransactions = async (sessionId: number): Promise<ImportSessionDetail> => {
  const response = await api.get(`/api/v1/transactions/import-sessions/${sessionId}/transactions`)
  return response.data
}
...
export const createCategoryRule = async (data: CategoryRuleCreate): Promise<CategoryRule> => {
  const response = await api.post('/api/v1/categories/rules', data)
  return response.data
}

export const updateCategoryRule = async (ruleId: number, data: CategoryRuleUpdate): Promise<CategoryRule> => {
...
): Promise<{ category_id: number | null; category_name: string | null; category_color: string | null }> => {
...
export const getSplitwiseCredentials = async (): Promise<SplitwiseCredentialsStatus> => {
...
export const updateSplitwiseCredentials = async (
  ...
  const response = await api.post('/api/v1/settings/splitwise/credentials', credentials)
  return response.data
}
...
export const createSplitwiseExpenses = async (
  ...
  const response = await api.post('/api/v1/settings/splitwise/expenses', request)
  return response.data
}
